...
When('eu adiciono o time {string} aos favoritos', (teamName) => {
  cy.intercept('GET', '**/team-favorite')
    .as('getFavorites');
...
When('eu removo o time {string} dos favoritos', (teamName) => {
  ...
  cy.get(`[title="${teamName}"]`)
    .parent()
    .find('.chakra-button.css-di3rc')
    .click();
...
When('eu navego para a aba de favoritos', () => {
  cy.intercept('GET', '**/_next/data/*/favoritos.json*')
    .as('getFavoritosData');
...
When('eu desativo o toggle "Partidas favoritas"', () => {
  cy.get('[data-cy="switch-favorites"]')
    .click();
});
