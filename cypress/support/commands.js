Cypress.Commands.add('login', (email, password, type) => {
  ...
  cy.intercept('GET', '**/favorite-matches')
    .as('favoriteMatches');
...
Cypress.Commands.add('addFavoriteTeam', (team) => {
  cy.get('body').then(($body) => {
    if ($body.find(`[title="${team}"]`).length === 0) {
      ...
      cy.intercept('GET', 'https://kasalive.api.prod.loomi.com.br/api/1.0/team/?page=1&name=*')
        .as('searchTeam');
...
Cypress.Commands.add('removeFavoriteTeam', (team) => { 
  ...
  cy.get(`[title="${team}"]`).then(() => {
    cy.get('.chakra-button.css-di3rc')
      .click();  
  });  
  cy.get('[data-cy="btn-save-teams"]')
    .click();
});